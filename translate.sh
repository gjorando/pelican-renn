...
# This script is blatantly stolen from Alexandre Vicenzi's Flex theme -> https://github.com/alexandrevicenzi/Flex
# I am not taking credit for it

function extract_pot() {
    pybabel extract -s --no-wrap --project="pelican-renn" --copyright-holder="Public domain" --version="1.0.0" --mapping translations/babel.cfg --output translations/messages.pot ./
}
...
function usage {
    echo "Pelican theme translation tool"
    echo "Originally made by Alexandre Vicenzi for the Flex pelican theme"
    echo "Default language is french because I am french (source: I am pedant and I stink)"
    echo ""
    echo "Usage:"
    echo ""
